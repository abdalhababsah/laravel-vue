...
const AddColor = async () => {
    ...
    try {
        await router.post("/admin/colors/store", data, {
            onSuccess: (page) => {
                Swal.fire({
                    toast: true,
...
const deleteColor = async (id) => {
    try {
        ...
        if (result.isConfirmed) {
            await router.delete(`/admin/colors/delete/${id}`, {
                onSuccess: (page) => {
                    Swal.fire({
                        toast: true,
                        title: page.props.flash.success,
...
const updateColor = async () => {
    ...
    try {
        await router.put(`/admin/colors/update/${id.value}`, data, {
            onSuccess: (page) => {
                dialogVisible.value = false;
                resetFormData();
                Swal.fire({
            ...
            <form class="px-6 py-4" @submit.prevent="editMode ? updateColor() : AddColor()">
                ...
                <div class="relative z-0 w-full mb-4">
                    ...
                    <input type="text" v-model="hex_code" id="hex-code-input"
                        class="w-full mt-2 border border-gray-300 rounded-lg p-2 text-gray-900 text-sm dark:bg-gray-600 dark:border-gray-500 dark:text-white"
                            ...
                            <tr v-if="colors.data && colors.data.length" v-for="color in colors.data" :key="color.id"
                                ...
                                <td class="px-4 py-3 ">
                                    ...
                                    <div :style="{ backgroundColor: color.hex_code || '#FFFFFF' }"
                                        ...
                                        <p>
                                    {{ color.hex_code ? color.hex_code : "No Hex" }}</p></div>
                                </td>
